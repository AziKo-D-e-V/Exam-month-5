  ...
  const checkUserFollow = await followersSchema.find({
    ...
    if (checkUserFollow.length < 0) {
      const costMonth = checkChannel.some(
        (data) => data.month === month && data.cost === payValue
      );
      ...
      if (payValue === undefined) {
        ...
        return res.json({
          message: "Successfully payed and subscriped this channel",
        });
      }
    } else {
      res
        .status(402)
        .json({ message: "Your account is already following this channel" });
    }
  ...
  try {
    ...
    if (user) {
      ...
      return res.status(200).json({
        message: `User status: ${
          user[0]?.status ? user[0].status : "deactive"
        }`,
    ...
    res
      .status(404)
      .json({ message: "Incorrect user_id", error: error.message });
  }
};
